...
public struct ZuuppaConfig: Sendable {
    /// Base URL of the Zuuppa API. Defaults to production.
    ...
    public var fields: ZuuppaCheckoutFields
    ...
    /// always come from the Zuuppa server). Set false to keep the sheet fully
    /// offline / avoid the third-party lookup.
    public var resolveTokenNames: Bool

    public init(
        apiBaseURL: URL = URL(string: "https://api.zuuppa.com")!,
        pollInterval: TimeInterval = 3.0,
        showPayWithWallet: Bool = true,
        payWithWalletTitle: String = "Pay with wallet",
        fields: ZuuppaCheckoutFields = .none,
